...
import { _decorator, Component, Node, ResolutionPolicy, Size, UITransform, view, Widget } from 'cc';
...
export default class ContentAdapter extends Component {
    onLoad() {
        ...
        const nodeUITr: UITransform = this.node.getComponent(UITransform);
        const nodeWidth: number = nodeUITr.width;
        const nodeHight: number = nodeUITr.height;
        const size: Size = view.getCanvasSize();
        let srcScaleForShowAll = Math.min(size.width / nodeWidth, size.height / nodeHight);
        let realWidth = nodeWidth * srcScaleForShowAll;
        let realHeight = nodeHight * srcScaleForShowAll;
